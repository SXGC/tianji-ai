...
export type { CausalContextProvider } from './causal-context-provider.js'
export { createAlsCausalContextProvider } from './causal-context-provider.js'
 ...
 * 3. contextProvider.update(child(eventId)) —— 推进因果链，后续事件自动继承
 *
 * @param deps - 依赖注入项（bus.publish、counter、contextProvider、source、recoverer）
 */
export function createRuntimeEventPipeline(deps: RuntimeEventPipelineDeps): RuntimeEventPipeline {
  ...
  return {
    async emitEvent(event: DomainEvent): Promise<void> {
      ...
      deps.publish(env)
      deps.contextProvider.update(deps.contextProvider.getCurrent().child(env.eventId))
    },
  }
}
