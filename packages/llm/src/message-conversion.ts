...
function convertSdkUserPartsToApp(content: unknown[]): MessagePart[] {
	...
	for (const part of content) {
		...
		if (typeof part === 'object' && part !== null && (part as { type?: unknown }).type === 'image') {
			...
			result.push(convertSdkImageToApp({ type: 'image', image: img, mimeType: (part as { mimeType?: string }).mimeType }))
			continue
		}

		// Check for reasoning (not allowed in user messages)
		if (typeof part === 'object' && part !== null && (part as { type?: unknown }).type === 'reasoning') {
			throw new ConversionError(
...
function convertSdkAssistantPartsToApp(content: unknown[]): MessagePart[] {
	...
	for (const part of content) {
		...
		if (typeof part === 'object' && part !== null && (part as { type?: unknown }).type === 'reasoning') {
			...
			result.push(convertSdkReasoningToApp({ type: 'reasoning', text: (part as { text: string }).text }))
			continue
		}
...
export function sdkMessageToAppMessage(
	...
	switch (message.role) {
		...
		default: {
			...
			throw new ConversionError(
				`Unsupported CoreMessage role: ${JSON.stringify(exhaustiveCheck)}`,
				'unsupported_role',
			)
		}
	}
