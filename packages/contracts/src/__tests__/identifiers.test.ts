/**
 * Tests for branded identifier types.
 *
 * These tests verify the runtime behavior of identifier factory functions
 * and type guards. The compile-time type safety (preventing plain string
 * assignment to branded types) is verified separately via TypeScript
 * compilation in the evidence file.
 */
import {
	createRunId,
	createSessionId,
...
describe('identifiers', () => {
	...
	describe('branded type isolation', () => {
		it('should not allow SessionId to be assigned to ThreadId at compile time', () => {
			// This test verifies that TypeScript enforces branded type isolation
			// At runtime, all identifiers are strings, but at compile time,
			// SessionId cannot be assigned to ThreadId without explicit casting
			const sessionId: SessionId = createSessionId('session-123')
			// The following line would cause a TypeScript error if uncommented:
			// const threadId: ThreadId = sessionId
			// Instead, we verify the types are distinct by checking that
		...
		it('should not allow RunId to be assigned to SessionId at compile time', () => {
			const runId: RunId = createRunId('run-789')
			// The following line would cause a TypeScript error if uncommented:
			// const sessionId: SessionId = runId
