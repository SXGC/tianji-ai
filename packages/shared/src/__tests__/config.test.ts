...
describe('config', () => {
	...
	describe('resolveEnvPlaceholder', () => {
		...
		it('should resolve valid placeholder', () => {
			expect(resolveEnvPlaceholder('${env:OPENAI_API_KEY}', mockResolver)).toBe(
				'sk-test-key',
			)
			expect(
				resolveEnvPlaceholder('${env:ANTHROPIC_API_KEY}', mockResolver),
			).toBe('ant-test-key')
		})

		it('should return original value if not a placeholder', () => {
			expect(resolveEnvPlaceholder('plain string', mockResolver)).toBe(
				'plain string',
			)
			expect(resolveEnvPlaceholder('sk-regular-key', mockResolver)).toBe(
				'sk-regular-key',
			)
		})
		...
		it('should throw for undefined env var', () => {
			expect(() =>
				resolveEnvPlaceholder('${env:UNDEFINED_VAR}', mockResolver),
	...
	describe('TianjiConfigSchema', () => {
		...
		it('should validate config with placeholders', () => {
			...
			const result = TianjiConfigSchema.parse(config)
			expect(result.llm?.providers?.openai?.apiKey).toBe(
				'${env:OPENAI_API_KEY}',
			)
		})
	...
	describe('DEFAULT_PATH_POLICY_CONFIG', () => {
		it('should have required fields', () => {
			expect(Array.isArray(DEFAULT_PATH_POLICY_CONFIG.forbidDirectories)).toBe(
				true,
			)
			expect(
				Array.isArray(DEFAULT_PATH_POLICY_CONFIG.filenameDenyPatterns),
			).toBe(true)
		})

		it('should include common forbidden directories', () => {
			expect(DEFAULT_PATH_POLICY_CONFIG.forbidDirectories).toContain('.git/')
			expect(DEFAULT_PATH_POLICY_CONFIG.forbidDirectories).toContain(
				'node_modules/',
			)
		})

		it('should include common deny patterns', () => {
			expect(DEFAULT_PATH_POLICY_CONFIG.filenameDenyPatterns).toContain(
				'^\\.env($|\\.)',
			)
			expect(DEFAULT_PATH_POLICY_CONFIG.filenameDenyPatterns).toContain(
				'(^|/)id_rsa$',
