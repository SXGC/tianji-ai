	...
	describe('deepClone', () => {
		describe('primitives', () => {
			...
			it('should clone numbers', () => {
				...
				expect(deepClone(Number.POSITIVE_INFINITY)).toBe(
	...
	describe('retry', () => {
		...
		describe('retry exhaustion', () => {
			it('should throw last error after all attempts fail', async () => {
				...
				await expect(
					retry(operation, { maxAttempts: 3, baseDelayMs: 10 }),
			...
			it('should wrap non-Error throws in Error', async () => {
				...
				await expect(
					retry(operation, { maxAttempts: 2, baseDelayMs: 10 }),
		...
		describe('options', () => {
			...
			it('should respect custom maxAttempts', async () => {
				...
				await expect(
					retry(operation, { maxAttempts: 5, baseDelayMs: 10 }),
				).rejects.toThrow()
